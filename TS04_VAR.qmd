...
\Rightarrow \bvarepsilon_t &= \bP \tilde{\bvarepsilon}_t .
\end{aligned}
$$
</div>
...
Now we can define the <span class="env-green">**Orthogonalized Impulse Response Function**</span> (OIRF) as
...
$$
\text{OIRF}(h) = \bPsi_h \bP = \begin{bmatrix}
\bpsi_{1h}' \\
\bpsi_{2h}' \\
\bpsi_{3h}'
\end{bmatrix}
...
\bpsi_{1h}' \bp_1 & \bpsi_{1h}' \bp_2 & \bpsi_{1h}' \bp_3 \\
\bpsi_{2h}' \bp_1 & \bpsi_{2h}' \bp_2 & \bpsi_{2h}' \bp_3 \\
\bpsi_{3h}' \bp_1 & \bpsi_{3h}' \bp_2 & \bpsi_{3h}' \bp_3
\end{bmatrix}
$$
...
$$
\begin{aligned}
\var\left[\by_{i, t+h} - \by_{i, t+h \mid t}\right] &= \sum_{\ell=0}^{h-1} \var \left[ \bpsi_{i\ell}' \bP \tilde{\bvarepsilon}_{t+h-\ell} \right] \\
&= \sum_{\ell=0}^{h-1} \bpsi_{i\ell}' \bP \E\left(\tilde{\bvarepsilon}_{t+h-\ell} \tilde{\bvarepsilon}_{t+h-\ell}'\right) \bP' \bpsi_{i\ell} \\
&= \sum_{\ell=0}^{h-1} \bpsi_{i\ell}' \bP \bP' \bpsi_{i\ell} 
\end{aligned}
$$ {#eq-fevd}

To isolate the contribution of the $j$-th shock to the forecast error variance, we can write the $h$-step forecast error in (-@eq-fevd) as
...
The $\text{FEVD}_{ij}(h)$ lies in $[0,1]$ and varies across $h.$
Small values indicate that $\tilde{\bvarepsilon}_{jt}$ contributes only a small amount to the variance of $\by_{it}.$
Large values indicate that $\tilde{\bvarepsilon}_{jt}$ contributes a large amount to the variance of $\by_{it}.$

A forecast error variance decomposition requires orthogonalized innovations. 
There is no non-orthogonalized version.
