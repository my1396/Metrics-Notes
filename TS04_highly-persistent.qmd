...
If $\{y_t: t=0,1,\ldots\}$ and $\{x_t: t=0,1,\ldots\}$ are two $\mathrm I(1)$ processes, then we would normally expect

$$
y_t - \beta x_t
$$

is an $\mathrm I(1)$ process (not stationary) for any number $\beta.$ 
Generally, if two series are integrated to different orders, then the linear combinations of them will be integrated to the higher of the two orders.


Nevertheless, it is *possible* that for some $\beta \ne 0,$ 

$$
\varepsilon_t = y_t - \beta x_t
$$ 

is an $\mathrm I(0)$ process, which means it has constant mean, constant variance, and autocorrelations that depend only on the time distance between any two variables in the series, and it is asymptotically uncorrelated.

If such a $\beta$ exists, we say that $y$ and $x$ are **cointegrated**, and we call $\beta$ the **cointegration parameter**, and $[1, -\beta]$ a **cointegration vector**.
...
The OLS estimator of $\beta$ from (-@eq-cointegrating-par) is called the <span class="env-green">**leads and lags estimator**</span> because it employs leads and lags of $\Delta x.$
...
## Error Correction Models {#sec-ecm}
...
<span class="env-green">If $y_t$ and $x_t$ are cointegrated with parameter $\beta,$</span> then we can include lags of
...
The terms <span class="env-green">$\delta s_{t-1} = \delta (y_{t-1} - \beta x_{t-1})$</span> is called the <span class="env-green">**error correction term**</span> and the model (-@eq-error_correction_model) is called the <span class="env-green">**Error Correction Model**</span>.
...
where $\delta<0,$ which ensures that the error correction term works to push $y$ (short-run dynamics) back toward the long-run equilibrium.

- If $y_{t-1} > \beta x_{t-1},$ then $y$ in the previous period has overshot the equilibrium. $\Rightarrow$ Because $\delta<0,$ the error correction term works to push $y$ back toward the equilibrium (downward adjustment).

- If $y_{t-1} < \beta x_{t-1},$ the error correction term works includes a positive change in $y$ back toward the equilibrium (upward adjustment).

$\Delta y_t = \alpha_0 + \gamma_0 \Delta x_t + u_t$ represents the long-run equilibrium relationship.


The cointegrating parameter $\beta$ can be estimated using the OLS estimator or the leads and lags estimator. 
@Engle1987 shows that the asymptotic efficiency of the estimators of the parameters in the error correction model is unaffected by the preliminary estimation of $\beta.$
...
<span class="h4">References:</span>

Section 18.4 Cointegration and Error Correction Models. Stock, James H., and Mark W. Watson. Introduction to Econometrics. 4th ed. Pearson Education Limited, 2019.
