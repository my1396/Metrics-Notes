...
Run the regression on data from 1962 to 1985, we obtain the following estimation.
...
The $t$-statistic on the slope coefficient is 5.66, which shows statistical significance.
...
Q: Why @eq-before1986 and @eq-after1986 generate so different but significant coefficient estimates?  
...
@fig-consup_income plotted logged consumption and logged income data. The figure suggests a highly significant relationship but the relationship is due to the fact that both series are trending over time. The regression results are spurious.
...
$$
\Delta y_t = \mu + \beta_0 \Delta x_{t-1} + (\gamma_1 - 1)(y_{t_1} - \theta x_{t-1}) + \varepsilon_t.
$$

- equilibrium relationship: $\Delta y_t = \mu + \beta_0 \Delta x_{t-1} + \varepsilon_t$, where $\varepsilon_t$ is the equilibrium error.
- error correction term: $(\gamma_1 - 1)(y_{t_1} - \theta x_{t-1})$

The standard approach is the Engle-Granger two-step method:

1. Run the cointegrating regression of $y_t$ on $x_t$ and save the residuals, $\hat{v}_t = y_t - \hat{\theta} x_t$.
2. Run the regression of $\Delta y_t$ on $\Delta x_t$ and $\hat{v}_{t-1}$ (the lagged residual is the error-correction term $y_{t-1}-\hat{\theta}x_{t-1}$).
...
data <- data %>%
    mutate(
        ...
        ecm_lag    = lag(ecm_term) # v_hat_{t-1}
    )
...
$$
\begin{aligned}
\Delta \text{REALCONS}_t &= 0.004 + 0.582 \times \Delta \text{REALGDP}_t + (0.905-1) \times (REALCONS - 1.057 REALGDP) \\
&\phantom{=} \;(0.00066) \;\; (0.05) \qquad\qquad\qquad\qquad (0.03) \qquad\qquad\qquad\qquad\qquad (0.002)
\end{aligned}
$$


```{r}
# Plot equilibrium errors
ecm_xts <- xts(data$ecm_term, qtr_index)
autoplot(ecm_xts) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
    labs(x = NULL, y = NULL, title = "Estimated Equilibrium Errors") +
    theme_minimal(base_size = 12) +
    theme(legend.position = "none")
```
