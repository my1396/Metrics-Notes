...
Notice that the RHS now includes the lagged dependent variables $y_{i,t-k},$ $k=1,\ldots,K.$ Adding dynamics to a model makes major changes in the interpretation of the equation.
...
The AH estimator delivers **consistent but not eﬃcient** estimates of the parameters in the model. This is due to the fact that the IV doesn't exploit all the available moments conditions.
...
- $t = 3$: the valid instrument for $\Delta y_{i2} = (y_{i2} - y_{i1})$ is $y_{i1}$,  
- $t = 4$: the valid instruments for $\Delta y_{i3} = (y_{i3} - y_{i2})$ are $y_{i2}$ as well as $y_{i1}$,  
- $t = 5$: the valid instruments for $\Delta y_{i4} = (y_{i4} - y_{i3})$ are $y_{i3}$ as well as $y_{i2}$ and $y_{i1}$,  
- $t = 6$: the valid instruments for $\Delta y_{i5} = (y_{i5} - y_{i4})$ are $y_{i4}$ as well as $y_{i3}$, $y_{i2}$, and $y_{i1}$,  
- $t = T$: the valid instruments for $\Delta y_{iT-1} = (y_{iT-1} - y_{iT-2})$ are $y_{iT-2}$ as well as $y_{iT-3}, \dots, y_{i1}$.

As $t$ increases, the number of instruments available also increases.

We'll have an instrument matrix with one row for each time period that we are instrumenting:
...
\quad
\Delta \varepsilon_{i\cdot} =
\begin{bmatrix}
\Delta \varepsilon_{i2} \\
\Delta \varepsilon_{i3} \\
\vdots \\
\Delta \varepsilon_{iT}
\end{bmatrix}.
$$

The number of rows of $Z_i$ is $T-3.$ The number of columns of $Z_i$ depends on the length of $T,$ it can grow very quickly with $T:$

$$
\sum_{k=1}^{T-2} k = \frac{(T-1)(T-2)}{2}
$$


@Arellano1991 suggested using a GMM approach based on all available conditions.
