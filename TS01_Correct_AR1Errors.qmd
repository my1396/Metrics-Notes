---
title: "Correcting for Serial Corealtion with Strictly Exogenous Regressors"
format: html
---
...
$$
\tilde{y_t} = \beta_1\tilde{x_{t1}} + \beta_2\tilde{x_{t2}} + \cdots + \beta_K\tilde{x_{tK}} + e_t,\; t\ge 2,
$${#eq-tranformed-model}
where
$$
\begin{split}
\tilde{y_t} &= y_t - \rho y_{t-1} \\
\tilde{x_{tj}} &= x_{tj} - \rho x_{t-1,j},\; j=1,\ldots,K. \\
\end{split}
$$
...
3. Apply the transformation as in (-@eq-tranformed-model), where
    ...
    $$
    \begin{split}
    \tilde{y_t} &= y_t - \hat\rho y_{t-1}, \; t\ge 2 \\
    \tilde{x_{tj}} &= x_{tj} - \hat\rho x_{t-1,j},\; j=1,\ldots,K. \\
    \end{split}
    $$
